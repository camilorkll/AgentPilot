...
{
    var (answer, documents, latency, cost) = await api.AskAsync(testCase.Question);
    ...
    var answerHit = testCase.ExpectedKeywords.Length == 0 ||
        testCase.ExpectedKeywords.Any(k => answer.Contains(k, StringComparison.OrdinalIgnoreCase));
...
report.AppendLine($"| Latencia media | {results.Average(r => r.LatencyMs):F0} ms |");
report.AppendLine($"| Latencia p95 | {Percentile(results.Select(r => (double)r.LatencyMs).ToList(), 0.95):F0} ms |");
...
report.AppendLine();
report.AppendLine("| # | Pregunta | Recuperación | Respuesta | Resultado |");
report.AppendLine("|---|---|---|---|---|");
foreach (var r in results)
{
    ...
    report.AppendLine($"| {r.Case.Id} | {r.Case.Question} | {retrieval} | {answerCell} | {(r.Passed ? "**OK**" : "FALLO")} |");
}
