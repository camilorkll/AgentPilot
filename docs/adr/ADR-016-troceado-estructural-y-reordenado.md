...
- **La puntuación del reordenado se publica** (`Citation.relevance`), aparte de la similitud (`Citation.score`). No se hizo desde el principio y se notaba: la lista de fuentes salía ordenada por una y etiquetada con la otra, así que un resultado podía mostrar menos similitud que el siguiente y la recuperación parecía rota sin estarlo. Enseñar solo el coseno escondía además el trabajo que hace este ADR — ante «¿se acumulan los datos no consumidos?», el fragmento que responde entra con similitud 0,310 y gana a otros de 0,379 y 0,357 porque su relevancia sube a 0,482. Sin ese número, el reordenado es invisible incluso cuando acierta.
