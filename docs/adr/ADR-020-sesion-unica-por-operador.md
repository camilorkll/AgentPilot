...
2. **La pantalla desplazada no se enteraba hasta actuar.** Es inherente a un token autocontenido sin canal de aviso: el JWT ya está en el navegador y nadie le dice nada. Seguía a la vista, aparentando sesión abierta. Se comprueba ahora al volver a ella; se descartó un temporizador para no pedir nada mientras nadie mira, con esta pantalla abierta toda la jornada.

   Hacen falta **dos** eventos, y con uno solo seguía fallando: `visibilitychange` cubre cambiar de pestaña o minimizar, pero **no** alternar entre dos ventanas — con las dos a la vista ninguna pestaña llega a ocultarse (`visibilityState` se queda en `visible`) y el evento no salta nunca. Y con dos ventanas es exactamente como se prueba esto. Lo cubre `focus` sobre `window`, con una guarda de un segundo porque cambiar de pestaña dispara los dos.
